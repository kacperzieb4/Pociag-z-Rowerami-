    ...
    while (1) {
        ...
        while ((time(NULL) - start_time < T || boarding_in_progress)){
            ...
            if (force_departure || (time(NULL) - start_time >= T)) {
                printf("\033[1;32m[KIEROWNIK PID=%d] Czas T minął lub wymuszony odjazd!\033[0m\n", train_ID);
            ...
            if (!block_passengers && pass_count < P) {
            ...
            if (!block_passengers && pass_count < P && bike_count < R) {
