...
void cleanup_passengers() {
    int cleanup_msq = get_message_queue(".", 4);  // Nowa kolejka dla usuwania pasażerów
    struct message msg;

    for (int i = 0; i < passenger_count; i++) {
        if (passenger_pids[i] > 0) {
            msg.mtype = 1;
            msg.ktype = passenger_pids[i];  // PID pasażera do usunięcia
            send_message(cleanup_msq, &msg);
        }
    }
    passenger_count = 0;
}
